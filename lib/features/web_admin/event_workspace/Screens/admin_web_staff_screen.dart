// lib/features/web_admin/event_workspace/Screens/admin_web_staff_screen.dart
...
class AdminWebStaffScreen extends StatefulWidget {
  final String eventId;
  final String eventName;

  const AdminWebStaffScreen({
    super.key,
  ...
  @override
  State<AdminWebStaffScreen> createState() =>
      _AdminWebStaffScreenState();
}

class _AdminWebStaffScreenState extends State<AdminWebStaffScreen> {
...
Future<void> _openEditStaffDialog({
  ...
  Widget build(BuildContext context) {
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          _StaffPageHeader(eventName: widget.eventName),
          ...
          StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
            ...
            builder: (context, snapshot) {
              ...
              final filtered = staffMembers.where((staffMember) {
                if (_searchQuery.isEmpty) return true;

                final data = staffMember.data();
              ...
              if (filtered.isEmpty) {
                return _MessageCard(
                  ...
                  title: staffMembers.isEmpty
                      ? 'No staff members found'
                      : 'No matching staff members',
                  message: staffMembers.isEmpty
                      ? 'No staff accounts are currently linked to ${widget.eventName}.'
              ...
              return _StaffTable(
                ...
                onEdit: (staffId, staffData) {
                  _openEditStaffDialog(
                    staffId: staffId,
                    staffData: staffData,
                  );
                },
...
class _StaffPageHeader extends StatelessWidget {
  final String eventName;

  const _StaffPageHeader({
    required this.eventName,
  });
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 7),
        const Text(
          'View staff information, edit profile fields, and change staff roles when needed.',
          style: TextStyle(
    ...
    return Container(
      ...
      child: TextField(
        ...
        decoration: InputDecoration(
          hintText:
              'Search staff by name, email, company, phone, country, or social profile',
...
class _StaffTable extends StatelessWidget {
  final List<QueryDocumentSnapshot<Map<String, dynamic>>> staffMembers;
  final void Function(
    String userId,
...
class _EditStaffDialog extends StatefulWidget {
  ...
  @override
  State<_EditStaffDialog> createState() =>
...
class _EditStaffDialogState extends State<_EditStaffDialog> {
  ...
  void initState() {
    ...
    _nameController = TextEditingController(
      text: (widget.staffData['name'] ?? '').toString(),
    );
    _emailController = TextEditingController(
      text: (widget.staffData['email'] ?? '').toString(),
    );
    _companyController = TextEditingController(
      text: (widget.staffData['company'] ?? '').toString(),
    );
    _positionController = TextEditingController(
      text: (widget.staffData['position'] ??
              widget.staffData['jobTitle'] ??
              '')
          .toString(),
    );
    _phoneController = TextEditingController(
      text: (widget.staffData['phone'] ??
              widget.staffData['phoneNumber'] ??
              '')
          .toString(),
    );
    _countryController = TextEditingController(
      text: (widget.staffData['country'] ?? '').toString(),
    );
    _bioController = TextEditingController(
      text: (widget.staffData['bio'] ?? '').toString(),
    );

    _linkedInController = TextEditingController(
      text: (widget.staffData['linkedin'] ??
              widget.staffData['linkedIn'] ??
              '')
          .toString(),
    );
    _twitterController = TextEditingController(
      text: (widget.staffData['twitter'] ?? '').toString(),
    );
    _websiteController = TextEditingController(
      text: (widget.staffData['website'] ?? '').toString(),
    );
    _githubController = TextEditingController(
      text: (widget.staffData['github'] ?? '').toString(),
    );
    _mediumController = TextEditingController(
      text: (widget.staffData['medium'] ?? '').toString(),
    );
    _instagramController = TextEditingController(
      text: (widget.staffData['instagram'] ?? '').toString(),
    );
    _profileImageController = TextEditingController(
      text: (widget.staffData['profileImageUrl'] ?? '').toString(),
    );

    final currentRole =
        (widget.staffData['role'] ?? 'staff')
            .toString()
            .toLowerCase();

    _selectedRole =
        _roles.containsKey(currentRole) ? currentRole : 'staff';

    final rawStatus =
        (widget.staffData['status'] ?? 'approved')
            .toString()
            .toLowerCase();
    ...
    final visibility =
        (widget.staffData['profileVisibility'] ?? 'full')
            .toString()
            .toLowerCase();
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (dialogContext) {
        return AlertDialog(
          ...
          content: Text(
            'This staff member will be changed from '
            '${_roles[oldRole] ?? oldRole} to '
            '${_roles[_selectedRole] ?? _selectedRole}.\n\n'
            'After saving, the staff member may move to another management page and disappear from the Staff page.',
          ),
          actions: [
            TextButton(
              onPressed: () =>
  ...
  Widget build(BuildContext context) {
    return _WebDialogFrame(
      ...
      title: 'Edit Staff',
      subtitle:
          'Review and update staff profile information for ${widget.eventName}.',
